<x-app-layout>
    <div class="bg-[#2A446C] h-full pl-72 pt-40">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 pb-20">
            ...
            <div class="bg-white rounded-xl shadow p-6 mb-6">
                <form method="GET" action="{{ route('laporan.produk') }}" class="flex flex-wrap gap-4 items-end">
                    <div>
            ...
            <div class="mb-6 w-1/3">
                <input type="text" id="searchLaporan" placeholder="Cari di semua laporan produk..."
            ...
            <div class="mb-10 section-laporan" id="section-masuk">
                <h2 class="text-2xl font-bold text-white mb-4">Laporan Produk Masuk</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-laporan">
                        <thead class="bg-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-2 text-center">Jumlah Masuk</th>
                                <th class="px-6 py-2 text-center">Petugas/Admin</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($logs->where('tipe_transaksi', 'masuk') as $log)
                                <tr class="row-data border-b border-gray-100 hover:bg-gray-50 transition-colors" data-jumlah="{{ $log->jumlah }}">
                                    ...
                                    <td class="text-center py-4">{{ $log->kode_produk }}</td>
                                    <td class="text-center py-4">{{ $log->nama_produk }}</td>
                                    <td class="text-center py-4">{{ $log->jumlah }}</td>
                                    <td class="text-center py-4">{{ $log->petugas ?? 'N/A' }}</td>
                                </tr>
                            @empty
                                <tr class="no-data">
                                    <td colspan="6" class="text-center py-10 text-gray-500 italic">Tidak ada data log masuk.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="mb-10 section-laporan" id="section-keluar">
                <h2 class="text-2xl font-bold text-white mb-4">Laporan Pengambilan Produk (Keluar)</h2>
                <div class="bg-white rounded-xl shadow overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200 table-laporan">
                        <thead class="bg-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-2 text-center">Penerima/Customer</th>
                        ...
                        <tbody>
                            @forelse($logs->where('tipe_transaksi', 'keluar') as $log)
                                <tr class="row-data border-b border-gray-100 hover:bg-gray-50 transition-colors" data-jumlah="{{ $log->jumlah }}">
                                    ...
                                    <td class="text-center py-4">{{ $log->nama_produk }}</td>
                                    <td class="text-center py-4">{{ $log->jumlah }}</td>
                                    <td class="text-center py-4">{{ $log->penerima ?? 'N/A' }}</td>
                                    <td class="text-center py-4">{{ $log->keterangan ?? 'N/A' }}</td>
                                </tr>
                            @empty
                                <tr class="no-data">
                                    <td colspan="7" class="text-center py-10 text-gray-500 italic">Tidak ada data log keluar.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="pb-10 section-laporan" id="section-retur">
                <h2 class="text-2xl font-bold text-white mb-4">Laporan Retur Produk</h2>
                <div class="bg-white rounded-xl shadow overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200 table-laporan">
                        <thead class="bg-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-2 text-center">Tindakan</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($logs->where('tipe_transaksi', 'retur') as $log)
                                <tr class="row-data border-b border-gray-100 hover:bg-gray-50 transition-colors" data-jumlah="{{ $log->jumlah }}">
                                    ...
                                    <td class="text-center py-4">{{ $log->nama_produk }}</td>
                                    <td class="text-center py-4">{{ $log->jumlah }}</td>
                                    <td class="text-center py-4">{{ $log->kondisi ?? 'N/A' }}</td>
                                    <td class="text-center py-4">{{ $log->keterangan ?? 'N/A' }}</td>
                                </tr>
                            @empty
                                <tr class="no-data">
                                    <td colspan="7" class="text-center py-10 text-gray-500 italic">Tidak ada data log retur.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="bg-white rounded-xl shadow p-4">
                <canvas id="chartProduk" width="400" height="200"></canvas>
            </div>
    ...
    <script>
        // 1. INISIALISASI CHART (Dibuat global agar bisa diupdate fungsi search)
        ...
        // 2. FUNGSI HITUNG ULANG DATA UNTUK CHART
